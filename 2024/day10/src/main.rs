...
fn main() -> anyhow::Result<()> {
	...
	fn traverse(grid: &Vec<Vec<u8>>, scores: &mut HashMap<((usize, usize), (usize, usize)), bool>, trailhead: (usize, usize), y: usize, x: usize) {
				...
				if next.is_ascii_alphanumeric() && current_height + 1 == next {
					if next == b'9' {
						*scores.entry((trailhead, (nexty, nextx))).or_default() = true;
						...
						traverse(&grid, scores, trailhead, nexty, nextx);
					}
				}
			}
		}
	...
	for y in 0..h {
		for x in 0..w {
			if grid[y][x] == b'0' {
				traverse(&grid, &mut scores, (y, x), y, x);
			}
		}
	}
